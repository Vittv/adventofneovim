...
print("add_one(2) -> ", add_one(2)) -- outputs add_one(2) -> 3
```
...
"#" is a length operator, which is indicating the end of the iteration on the first for loop.

```lua
local reading_scores = { teej_dv = 10, ThePrimeagen = "N/A" }
for index = 1, #readingscores do
...
Lua doesn't have default arguments, so they have to be explicitly declared, as shown above.
